...
public class GraphicsFactory {
	...
	private static final Logger logger = LoggerFactory.getLogger(GraphicsFactory.class);
	 ...
	 * by the card
	 * 
	 * @throws GameException Indicates no buffers are supported
	 */
	private static void init() throws GameException {
		init = true;
		...
		if (!fbo) {
			throw new GameException("Your OpenGL card does not support offscreen buffers and hence can't handle the dynamic images required for this application.");
		}

		logger.info("Offscreen Buffers FBO="+fbo+" PBUFFER="+pbuffer+" PBUFFERRT="+pbufferRT);
	}
	 ...
	 * @throws GameException Indicates it wasn't possible to create a graphics context
	 * given available hardware.
	 */
	public static Graphics getGraphicsForImage(Image image) throws GameException {
	 ...
	 * @throws GameException Indicates a failure to release the context
	 */
	public static void releaseGraphicsForImage(Image image) throws GameException {
	 ...
	 */
	private static Graphics createGraphics(Image image) throws GameException {
		init();
		...
		if (fbo) {
			try {
				...
				fbo = false;
				logger.warn("FBO failed in use, falling back to PBuffer");
			}
		}
		
		throw new GameException("Failed to create offscreen buffer even though the card reports it's possible");
	}
}
