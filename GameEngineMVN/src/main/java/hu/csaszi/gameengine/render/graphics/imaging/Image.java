...
import hu.csaszi.gameengine.game.exception.GameException;
import hu.csaszi.gameengine.render.core.Color;
import hu.csaszi.gameengine.render.core.gl.Graphics;
import hu.csaszi.gameengine.render.core.gl.pbuffer.GraphicsFactory;
import hu.csaszi.gameengine.render.core.gl.renderer.*;
import hu.csaszi.gameengine.util.ImageBuffer;
...
public class Image implements Renderable {
	...
	protected static GL GEGL = Renderer.get();
	...
	private int filter = GL.GL_LINEAR;
	 ...
	 */
	public Image(String ref) throws GameException {
		this(ref, false);
	}
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	public Image(String ref, Color trans) throws GameException  {
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	public Image(String ref, boolean flipped) throws GameException {
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	public Image(String ref, boolean flipped, int filter) throws GameException {
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	public Image(String ref, boolean flipped, int f, Color transparent) throws GameException {
		this.filter = f == FILTER_LINEAR ? GL.GL_LINEAR : GL.GL_NEAREST;
		...
		try {
			...
			throw new GameException("Failed to load image from: "+ref, e);
		}
	}
	...
	public void setFilter(int f) {
		this.filter = f == FILTER_LINEAR ? GEGL.GL_LINEAR : GEGL.GL_NEAREST;

		texture.bind();
		GEGL.glTexParameteri(GEGL.GL_TEXTURE_2D, GEGL.GL_TEXTURE_MIN_FILTER, filter);
		GEGL.glTexParameteri(GEGL.GL_TEXTURE_2D, GEGL.GL_TEXTURE_MAG_FILTER, filter);
	}
	 ...
	 * @throws GameException Indicates a failure to create the underlying resource
	 */
	public Image(int width, int height) throws GameException {
	 ...
	 * @throws GameException Indicates a failure to create the underlying resource
	 */
	public Image(int width, int height, int f) throws GameException {
		ref = super.toString();
		this.filter = f == FILTER_LINEAR ? GEGL.GL_LINEAR : GEGL.GL_NEAREST;
		...
		try {
			...
			logger.error(e.getMessage(), e);
			throw new GameException("Failed to create empty image "+width+"x"+height);
		}
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	public Image(InputStream in, String ref, boolean flipped) throws GameException {
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	public Image(InputStream in, String ref, boolean flipped,int filter) throws GameException {
	...
	public void clampTexture() {
		if (GEGL.canTextureMirrorClamp()) {
			GEGL.glTexParameteri(GEGL.GL_TEXTURE_2D, GEGL.GL_TEXTURE_WRAP_S, GEGL.GL_MIRROR_CLAMP_TO_EDGE_EXT);
			GEGL.glTexParameteri(GEGL.GL_TEXTURE_2D, GEGL.GL_TEXTURE_WRAP_T, GEGL.GL_MIRROR_CLAMP_TO_EDGE_EXT);
		} else {
			GEGL.glTexParameteri(GEGL.GL_TEXTURE_2D, GEGL.GL_TEXTURE_WRAP_S, GEGL.GL_CLAMP);
			GEGL.glTexParameteri(GEGL.GL_TEXTURE_2D, GEGL.GL_TEXTURE_WRAP_T, GEGL.GL_CLAMP);
		}
	}
	 ...
	 * @throws GameException Indicates a failure to create a graphics context
	 */
	public Graphics getGraphics() throws GameException {
	 ...
	 * @throws GameException Indicates a failure to load the image
	 */
	private void load(InputStream in, String ref, boolean flipped, int f, Color transparent) throws GameException {
		this.filter = f == FILTER_LINEAR ? GEGL.GL_LINEAR : GEGL.GL_NEAREST;
	...
	public void drawEmbedded(float x,float y,float width,float height) {
		...
		if (corners == null) {
			GEGL.glTexCoord2f(textureOffsetX, textureOffsetY);
			GEGL.glVertex3f(x, y, 0);
			GEGL.glTexCoord2f(textureOffsetX, textureOffsetY + textureHeight);
			GEGL.glVertex3f(x, y + height, 0);
			GEGL.glTexCoord2f(textureOffsetX + textureWidth, textureOffsetY
					+ textureHeight);
			GEGL.glVertex3f(x + width, y + height, 0);
			GEGL.glTexCoord2f(textureOffsetX + textureWidth, textureOffsetY);
			GEGL.glVertex3f(x + width, y, 0);
		} else {
	...
	public void drawSheared(float x,float y, float hshear, float vshear, Color filter) {
		...
		GEGL.glTranslatef(x, y, 0);
		if (angle != 0) {
			GEGL.glTranslatef(centerX, centerY, 0.0f);
			GEGL.glRotatef(angle, 0.0f, 0.0f, 1.0f);
			GEGL.glTranslatef(-centerX, -centerY, 0.0f);
		}

		GEGL.glBegin(GEGL.GL_QUADS);
		init();

		GEGL.glTexCoord2f(textureOffsetX, textureOffsetY);
		GEGL.glVertex3f(0, 0, 0);
		GEGL.glTexCoord2f(textureOffsetX, textureOffsetY + textureHeight);
		GEGL.glVertex3f(hshear, height, 0);
		GEGL.glTexCoord2f(textureOffsetX + textureWidth, textureOffsetY
				+ textureHeight);
		GEGL.glVertex3f(width + hshear, height + vshear, 0);
		GEGL.glTexCoord2f(textureOffsetX + textureWidth, textureOffsetY);
		GEGL.glVertex3f(width, vshear, 0);
		GEGL.glEnd();
	...
	public void drawFlash(float x,float y,float width,float height, Color col) {
		...
		if (GEGL.canSecondaryColor()) {
			GEGL.glEnable(GEGL.GL_COLOR_SUM_EXT);
			GEGL.glSecondaryColor3ubEXT((byte)(col.r * 255),
					(byte)(col.g * 255),
					(byte)(col.b * 255));
		}

		GEGL.glTexEnvi(GEGL.GL_TEXTURE_ENV, GEGL.GL_TEXTURE_ENV_MODE, GEGL.GL_MODULATE);
		...
		if (GEGL.canSecondaryColor()) {
			GEGL.glDisable(GEGL.GL_COLOR_SUM_EXT);
		}
	}
...
//		GL.glEnd();
	...
	public void drawEmbedded(float x, float y, float x2, float y2, float srcx, float srcy, float srcx2, float srcy2, Color filter) {
		...
		GEGL.glTexCoord2f(newTextureOffsetX, newTextureOffsetY);
		GEGL.glVertex3f(x,y, 0.0f);
		GEGL.glTexCoord2f(newTextureOffsetX, newTextureOffsetY
				+ newTextureHeight);
		GEGL.glVertex3f(x,(y + myheight), 0.0f);
		GEGL.glTexCoord2f(newTextureOffsetX + newTextureWidth,
				newTextureOffsetY + newTextureHeight);
		GEGL.glVertex3f((x + mywidth),(y + myheight), 0.0f);
		GEGL.glTexCoord2f(newTextureOffsetX + newTextureWidth,
				newTextureOffsetY);
		GEGL.glVertex3f((x + mywidth),y, 0.0f);
	}
	...
	public void drawWarped(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
		...
		GEGL.glVertex3f(0, 0, 0);
		GEGL.glTexCoord2f(textureOffsetX, textureOffsetY + textureHeight);
		GEGL.glVertex3f(x2 - x1, y2 - y1, 0);
		GEGL.glTexCoord2f(textureOffsetX + textureWidth, textureOffsetY
				+ textureHeight);
		GEGL.glVertex3f(x3 - x1, y3 - y1, 0);
		GEGL.glTexCoord2f(textureOffsetX + textureWidth, textureOffsetY);
		GEGL.glVertex3f(x4 - x1, y4 - y1, 0);
		GEGL.glEnd();
	 ...
	 *
	 * @throws GameException Indicates a failure to release resources on the graphics card
	 */
