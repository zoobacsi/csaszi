...
public class OpenALStreamPlayer {
	...
	private static final Logger logger = LoggerFactory.getLogger(OpenALStreamPlayer.class);
	...
	protected OAudioInputStream audio;
	...
	private void initStreams() throws IOException {
		...
		OggInputStream audiof;
		
		if (url != null) {
			audiof = new OggInputStream(url.openStream());
		} else {
			audiof = new OggInputStream(ResourceLoader.getResourceAsStream(ref));
		}
		...
		try {
			...
			if (count != -1) {
				...
				try {
					...
					logger.error("Failed to loop buffer: "+bufferId+" "+format+" "+count+" "+audio.getRate(), e);
					return false;
				}
			} else {
				if (loop) {
