 ...
 * @author kevin
 */
public class OggInputStream extends InputStream implements OAudioInputStream {

	private static final Logger logger = LoggerFactory.getLogger(OggInputStream.class);
	...
	private boolean getPageAndPacket() {
		...
		try {
			...
			logger.error("Failure reading in vorbis", e);
			endOfStream = true;
			return false;
		}
		...
		if (streamState.pagein(page) < 0) {
			...
			logger.error("Error reading first page of Ogg bitstream data.");
			endOfStream = true;
			return false;
		}
		...
		if (streamState.packetout(packet) != 1) {
			...
			logger.error("Error reading initial header packet.");
			endOfStream = true;
			return false;
		}
		...
		if (oggInfo.synthesis_headerin(comment, packet) < 0) {
			...
			logger.error("This Ogg bitstream does not contain Vorbis audio data.");
			endOfStream = true;
			return false;
		}
		...
		while (i < 2) {
				...
				if (result == 1) {
					...
					while (i < 2) {
						...
						if (result == -1) {
							...
							logger.error("Corrupt secondary header.  Exiting.");
							endOfStream = true;
							return false;
						}
			...
			try {
				...
				logger.error("Failed to read Vorbis: ", e);
				endOfStream = true;
				return false;
			}
			if (bytes == 0 && i < 2) {
				logger.error("End of file before finding all Vorbis headers!");
				endOfStream = true;
				return false;
			}
			syncState.wrote(bytes);
			...
			while (!endOfBitStream) {
				while (!endOfBitStream) {
					...
					if (result == -1) { // missing or corrupt data at this page position
						logger.error("Corrupt or missing data in bitstream; continuing...");
					} else {
				...
				if (!endOfBitStream) {
					...
					if (index >= 0) {
						...
						try {
							...
							logger.error("Failure during vorbis decoding", e);
							endOfStream = true;
							return;
						}
					} else {
