...
public class SoundStore {
	...
	private static final Logger logger = LoggerFactory.getLogger(SoundStore.class);
	...
	public void init() {
		...
		logger.info("Initialising sounds..");
		inited = true;
		...
		AccessController.doPrivileged(new PrivilegedAction() {
            public Object run() {
				try {
					...
					sounds = true;
					music = true;
					logger.info("- Sound works");
				} catch (Exception e) {
					logger.error("Sound initialisation failure.", e);
					soundWorks = false;
					sounds = false;
					music = false;
				}
		...
		if (soundWorks) {
			...
			logger.info("- "+sourceCount+" OpenAL source available");
		
			if (AL10.alGetError() != AL10.AL_NO_ERROR) {
				sounds = false;
				music = false;
				soundWorks = false;
				logger.error("- AL init failed");
			} else {
				...
				logger.info("- Sounds source generated");
			}
		}
	}
	...
	public Audio getOgg(String ref, InputStream in) throws IOException {
		...
		if (loaded.get(ref) != null) {
			...
			try {
				...
				logger.error("Error","Failed to load: "+ref+" - "+e.getMessage(), e);
				throw new IOException("Unable to load: "+ref);
			}
		}
		...
		if (music) {
			if (mod != null) {
				try {
					...
					logger.error("Error with OpenGL MOD Player on this this platform", e);
					mod = null;
				}
			}
			if (stream != null) {
				try {
					...
					logger.error("Error with OpenGL Streaming Player on this this platform", e);
					mod = null;
				}
			}
		}
